# CLAUDE.md full history (archived 2026-09-26)

Verbatim copy of the root CLAUDE.md as of commit bf10a37b, before it was condensed from 31KB to under 10KB
(context-window hygiene: it loaded on every prompt). Every rule survives in the new CLAUDE.md in short form;
this file keeps the incident narratives, evidence and commit ids. Read it when a rule's WHY matters.
...
- **Any scheduled/headless audit session MUST commit + push the audit files it writes via `bash scripts/audit-commit-guard.sh -m "<msg>" <paths…>` before ending — never a bare `git commit`.** `audit-snapshots.json` is built by the apps/web prebuild from COMMITTED audit files only; uncommitted audit runs silently freeze the GSC indexing snapshot. Bit us identically twice (2026-06-11, 2026-07-13), then a third time on 2026-07-29 in a new way — and the third time exposed that "just commit it" was never sufficient:
  ...
  - **Committing an audit could never, by itself, trigger the build that consumes it.** `apps/web/vercel-ignore.sh` excluded `gsc-audits/` and every `.md`, so audit-only commits were skipped at the ignore step (`errorLink: ignored-build-step`) — past audits only built by riding along on unrelated code commits. **Fixed at source 2026-07-30:** the ignore script now tests `gsc-audit-*.md` / `ga4-audit-*.md` FIRST and always builds for them, at the deliberate cost of one extra build per audit day. Do not re-add those paths to its exclude list. **Throttled 2026-09-25:** audit-only changes now build only if the last deployed commit is ≥20h old (`AUDIT_BUILD_MIN_HOURS`); a skipped audit rides the next build because `VERCEL_GIT_PREVIOUS_SHA` doesn't move. Reason: 42 of 85 prod builds (Aug 24–Sep 23) were audit-only, and **every deploy empties the ISR cache** (verified: 19/20 random dest×month pages MISS an hour after a deploy, none older than it), so crawlers regenerate ~6.5K pages/day — the ISR-writes + origin-transfer lines of a $73.55 bill. The snapshot's only consumers are two once-daily alert crons, so one audit build/day is enough. `ops/` is also skipped now (nothing in apps/web reads it). Measure on the 2026-10-24 invoice: usage (ex $20 Pro) was $53.55, target ≤ $40.
  ...
  - **6th occurrence, 2026-09-09/10 — cron cannot push AT ALL, and this is now a LaunchAgent.** The 09-09 audit committed and then failed to push with the *same* message as the 5th occurrence (`gh auth git-credential get` → `could not read Username for 'https://github.com': Device not configured`), stranding `f3fd14ea`. **It is NOT the PATH scar** — verified 09-09 that both scripts export `/opt/homebrew/bin` and that `/opt/homebrew/bin/gh` exists. The real cause: `gh` stores its token in the **macOS login keyring**, and cron runs outside the user's Aqua session so it cannot unlock the keychain. The credential helper therefore returns nothing. **Proven by experiment, both directions:** a LaunchAgent (`com.ashish.automation-backup`) pushed successfully to a private remote using the same helper, while cron could not; and conversely a LaunchAgent running `/usr/bin/python3` got `Operation not permitted` reading `~/Desktop` while cron read it fine. TCC is **per-executable**: `/bin/bash` already holds Full Disk Access on this machine, `/usr/bin/python3` does not. **Fix applied 2026-09-10: the job moved from crontab to `com.ashish.ga4-audit`** (plist kept in-repo at `scripts/com.ashish.ga4-audit.plist`, runs `/bin/bash scripts/ga4-audit-cron.sh` at 09:45 + 14:45). The crontab line is commented out with the reason — **do NOT re-enable it while the LaunchAgent is loaded, they would collide.** Verified end-to-end: audit written, committed `3e551e2f`, pushed, guard reported all checks passed. Rule of thumb this leaves behind: **a scheduled job that must read `~/Desktop` AND `git push` has to be a LaunchAgent invoked through `/bin/bash`** — cron gets the files but not the keychain, and launchd via a non-FDA interpreter gets the keychain but not the files.
  - **5th occurrence, 2026-08-26 — the wrapper was right; cron's PATH was not.** For four consecutive days (08-23 → 08-26) `ga4-audit-cron.sh` committed cleanly and then failed to push, stranding every commit locally. The cron log said only `gh auth git-credential get: gh: command not found` followed by git's generic `could not read Username for 'https://github.com': Device not configured` — which reads like an expired token, and is why it survived four days of daily QA. **The real cause: git in this repo authenticates through `credential.helper = !gh auth git-credential`, so `git push` needs `gh` on PATH, and cron's PATH did not include `/opt/homebrew/bin`.** The lesson generalises past this repo: **a cron script's PATH must cover its tools' tools, not just the binaries it calls by name.** `node` and `git` were both present; the thing that was missing was reached only by git's own config. Fixed in `e04cccec` — both scripts export a PATH including `/opt/homebrew/bin` and `~/bin`, and `audit-commit-guard.sh` now preflights the credential helper so this class names itself instead of hiding behind a generic auth error. Diagnose this class with `printf 'protocol=https\nhost=github.com\n\n' | env -i HOME="$HOME" PATH="<cron's PATH>" git credential fill`, which reproduces it in one command. Second lesson from the same fix: **the heal branch that was supposed to catch exactly this was dead code** — it sat inside the `no uncommitted audit files` branch, and since the audit writes a file every single day that branch never ran. A self-healing path that cannot be reached by the failure it heals is worse than none, because it reads as coverage. It now runs unconditionally after either path and verifies the remote actually moved.
...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-09-07 (12th consecutive week of the disable recommendation, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) still show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). No fresh Googlebot crawls this week — all 5 last-crawl timestamps unchanged from 08-31. No Request Indexing action taken — all 5 already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (12th re-run, `scripts/_gsc-ctr-check-2026-09-07.mjs`, RECENT window now Aug 11–Sep 7): still unreadable at the query level for all 6 target queries — zero clicks in every window, every run, unchanged (12 straight checks, ~3.5 months). Page level: `yercaud/may`'s decline is now a **confirmed 4-week monotonic worsening trend** (position 11.3→16.4→29.3→**41.3** across 08-17/08-24/08-31/09-07, impressions 77→69→41→28), a 2nd straight zero-click week — this is the clearest, longest-running signal the task has produced; `vrindavan/may` gave back most of last week's surprise gain (20.5→11.0→**13.3**); `darjeeling/june` hit its lowest position yet (36.0→**63.0** on just 1 impression, noisy sample); `chakrata/may` broke its 4-week zero-impression streak with 3 impressions; `pondicherry/may` stayed at 0 impressions for a 2nd week. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM) **for the 12th consecutive time** — the consolidation question itself is 12-for-12 unchanged and already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons; the yercaud decline is now a real 4-week trend worth a manual look regardless of this task's fate. Left the task enabled since this run's brief didn't authorize disabling automations — founder decision needed to actually turn it off (see `gsc-audits/scheduled-2026-09-07-canonical-consolidation.md`).
