...
- **Git divergence handling (see full detail below):** the shared working tree was found `ahead 2, behind 6` of `origin/main`, with a leftover locked worktree/branch (`audit-wt` / `audit-push-2026-08-16`) abandoned mid-cleanup by a prior sandbox session, plus several stale `.git/*.lock` files. Resolved via Desktop Commander (the sandbox bash cannot unlink files under `.git/*` on this mounted repo — a known, previously-documented bridge limitation) rather than fighting the sandbox. Full mechanics of what was done — lock clearing, stale-worktree cleanup, and the cherry-pick-onto-origin push technique used to land this audit's commit without disturbing two unrelated, already-known-stranded local QA commits — are recorded inline in this session and match the established playbook for this repo.
