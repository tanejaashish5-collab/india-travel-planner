...
function RoutesContent({ routes }: { routes: any[] }) {
  ...
  // Group by duration
  const grouped = {
    "Weekend (3-4 days)": routes.filter((r) => r.days <= 4),
    "Week (7 days)": routes.filter((r) => r.days >= 5 && r.days <= 7),
    "Extended (8-12 days)": routes.filter((r) => r.days >= 8),
  };

  return (
    <>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">{tn("routes")}</h1>
        <p className="mt-1 text-muted-foreground">
          {routes.length} curated itineraries from 3-day weekends to 12-day road
          trips
        </p>
      </div>
                ...
                {groupRoutes.map((route: any) => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                      <span
                        className={`rounded px-1.5 py-0.5 ${DIFFICULTY_COLORS[route.difficulty] ?? ""}`}
                      >
                        {route.difficulty}
                      </span>
                      {route.budget_range && (
