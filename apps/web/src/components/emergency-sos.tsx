...
// A phone value is only dialable if it carries at least one digit. Some SOS
// rows use the phone field for routing prose ("Contact via X PS") — those must
// never render a dead tel: link.
function isDialable(phone?: string | null): boolean {
                    ...
                    {sos.local_helpers?.map((helper, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-xl border border-border/50 p-3">
                        ...
                        {isDialable(helper.phone) ? (
                          ...
                          helper.phone && (
                            <span className="text-[10px] text-muted-foreground/70 text-right max-w-[42%] leading-tight">
                              {helper.phone}
                            </span>
                          )
                        )}
...
export function SOSFloatingButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="fixed bottom-36 left-4 md:bottom-8 md:left-4 z-30 md:z-50 flex h-14 w-14 items-center justify-center rounded-full bg-red-600 text-white shadow-lg shadow-red-600/30 hover:bg-red-700 transition-all animate-pulse-subtle"
