...
// Global event name. Any component (nav button, inline destination CTA) can
// open the chat by dispatching `new Event(ASK_NAKSHIQ_OPEN_EVENT)` on window
// — keeps things decoupled without pulling in a state library.
export const ASK_NAKSHIQ_OPEN_EVENT = "asknakshiq:open";

export function AskNakshIQ({ hideTrigger = false }: { hideTrigger?: boolean } = {}) {
  ...
  // Listen for global open events from nav button + inline destination CTAs.
  useEffect(() => {
    function onOpen() { setOpen(true); }
    window.addEventListener(ASK_NAKSHIQ_OPEN_EVENT, onOpen);
  ...
  return (
    <>
      {/* Floating trigger button — only rendered when the consumer hasn't
          opted into hideTrigger. Default mount in layout.tsx now passes
          hideTrigger so the chat is opened from the nav '?' icon and from
          inline destination CTAs instead. Keeps mobile real estate for
          the Plan My Trip pill. */}
      {!hideTrigger && (
        <button
          onClick={() => setOpen((v) => !v)}
          className={`fixed z-[1000] flex items-center gap-2 rounded-full shadow-2xl transition-all duration-300 ${
            open
              ? "bottom-[500px] md:bottom-[520px] left-4 md:left-6 bg-muted/80 px-3 py-2 text-muted-foreground hover:bg-muted"
              : "bottom-[calc(4.5rem+env(safe-area-inset-bottom))] md:bottom-6 left-4 md:left-6 bg-primary px-4 py-3 text-primary-foreground hover:shadow-primary/30 hover:-translate-y-0.5"
          }`}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] md:bottom-6 left-4 md:left-6 z-[1000] w-[calc(100vw-2rem)] sm:w-[400px] h-[min(440px,calc(100vh-6rem-env(safe-area-inset-bottom)))] md:h-[480px] rounded-2xl border border-border bg-card shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
