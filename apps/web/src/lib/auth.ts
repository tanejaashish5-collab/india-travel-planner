...
export async function syncLocalDataToProfile(userId: string) {
  ...
  // Gap Year plan sync removed 2026-08-04 with the feature itself: the surface
  // produced zero plans in its entire lifetime (gap_year_plans was empty) and
  // its generator was the metered Anthropic path we are retiring. The table is
  // left in place so nothing is destroyed; only the code path is gone.
}
