...
export default function RoutesScreen() {
  ...
  if (loading) return <View style={s.center}><Stack.Screen options={{ title: "Routes" }} /><ActivityIndicator size="large" color={colors.primary} /></View>;

  return (
    <View style={s.container}>
      <Stack.Screen options={{ title: `${routes.length} Road Trip Routes` }} />
      <View style={s.chipRow}>
        {[{id:"all",label:"All"},{id:"short",label:"≤5 days"},{id:"long",label:"8+ days"},{id:"bike",label:"🏍 Bike"},{id:"kids",label:"👶 Kids"}].map((f) => (
      ...
      <FlatList
        ...
        renderItem={({ item: r }) => (
          <TouchableOpacity style={s.card} onPress={() => router.push(`/route/${r.id}`)} activeOpacity={0.85}>
            {/* Header */}
            ...
            <View style={s.badgeRow}>
              {r.difficulty && <View style={[s.badge, { backgroundColor: (DIFF_COLOR[r.difficulty] || colors.muted) + "20" }]}><Text style={[s.badgeText, { color: DIFF_COLOR[r.difficulty] || colors.foreground }]}>{r.difficulty}</Text></View>}
              {r.bike_route && <View style={[s.badge, { backgroundColor: colors.score4 + "20" }]}><Text style={[s.badgeText, { color: colors.score4 }]}>🏍 Bike route</Text></View>}
              {r.kids_suitable && <View style={[s.badge, { backgroundColor: colors.score5 + "20" }]}><Text style={[s.badgeText, { color: colors.score5 }]}>👶 Kids OK</Text></View>}
              {r.budget_range && <View style={[s.badge, { backgroundColor: colors.muted }]}><Text style={s.badgeText}>{r.budget_range}</Text></View>}
            </View>
              ...
              <View style={s.stopsRow}>
                {r.stops.map((stop, i) => (
                  <View key={i} style={s.stopItem}>
                    ...
                    <Text style={s.stopText} numberOfLines={1}>{stop}</Text>
            ...
            {r.highlights?.length > 0 && (
              <Text style={s.highlights} numberOfLines={1}>{r.highlights.slice(0, 3).join(" · ")}</Text>
            )}

            {/* Best months */}
            {r.best_months?.length > 0 && (
              <View style={s.monthRow}>
                {r.best_months.map((m) => <View key={m} style={s.monthPill}><Text style={s.monthText}>{MONTH_SHORT[m]}</Text></View>)}
              </View>
            )}
          </TouchableOpacity>
        )}
...
const s = StyleSheet.create({
  ...
  chip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: borderRadius.full, borderWidth: 1, borderColor: colors.border },
  ...
  cardDesc: { fontSize: fontSize.sm, color: colors.mutedForeground, marginTop: 4, lineHeight: 20 },
  daysCircle: { width: 52, height: 52, borderRadius: 26, backgroundColor: colors.primary + "15", justifyContent: "center", alignItems: "center" },
  ...
  badgeText: { fontSize: 11, fontWeight: "600", color: colors.foreground, textTransform: "capitalize" },
  stopsRow: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", marginTop: spacing.md, gap: 2 },
  stopItem: { flexDirection: "row", alignItems: "center" },
  stopDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.score4, marginRight: 4 },
