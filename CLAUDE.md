...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-08-10 (8th consecutive week of the disable recommendation, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) still show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). No fresh Googlebot re-crawl on any of the 5 this week — every `lastCrawlTime` is unchanged from 08-03, breaking the "one URL gets re-checked each week" pattern of the prior two runs; not a concern, just nothing new for Google to confirm. No Request Indexing action taken — the URLs are already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (8th re-run, `scripts/_gsc-ctr-check-2026-08-10.mjs`): still unreadable for 5 of 6 target queries (hard-seasonal "X in may" searches, 0% pre-window CTR baseline, re-test May 2027); zero clicks on all 6 named queries in every window, every run, unchanged. Two real movements this week, neither changing the action outcome: `vrindavan/may` page position **reversed its three-week drift**, moving from 30.8 (08-03) to **24.3 (08-10)** — best reading since tracking began, flagged without declaring it "fixed" given the 08-03 run's experience correcting an over-confident "stabilized" call; `yercaud/may` logged a **second consecutive page-level click** (84 impr, 1 clk, 1.19% CTR, following last week's first-ever click) — still not attributable to the named query itself (0 clicks there), but now a repeated rather than one-off signal. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM) **for the 8th consecutive time**: the 2026-06-22 through 08-03 runs already reached the same operational conclusion (no action needed) — this week's new data points (vrindavan reversal, yercaud's second click) don't change that. Ongoing indexing health is already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons. Left the task enabled since this run's brief didn't authorize disabling automations — founder decision needed to actually turn it off (see `gsc-audits/scheduled-2026-08-10-canonical-consolidation.md`).
- 🔍 **Unverified anomaly, needs a founder look: `dotenvx` CLI printed a tip pointing at an unfamiliar domain during the 2026-08-10 canonical-consolidation run.** Every other `dotenvx` self-promo "tip" line seen this session (2 of 3 invocations, 6 tip-lines total) referenced either a CLI flag or the real `www.dotenvx.com`; one line from the batch `gsc-inspect-sweep.mjs --url` call read `⌁ auth for agents [www.vestauth.com]` instead. The string doesn't appear anywhere in the installed `node_modules/@dotenvx/dotenvx` source, so it's not a local file tamper — either a rotating remote-fetched tip (dotenvx does phone home for these) or something injected at another layer. Not treated as a confirmed compromise; not acted on (no link followed). `@dotenvx/dotenvx@1.60.1` comes in transitively via `shadcn@4.1.2` in `apps/web`. See "Notes on autonomous decisions" in `gsc-audits/scheduled-2026-08-10-canonical-consolidation.md` for the full trail. Worth a human glance — a quick search of dotenvx's issue tracker, or just noting it and moving on if it doesn't recur.
