...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-08-31 (11th consecutive week of the disable recommendation, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) still show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). `yercaud/may` got a fresh Googlebot crawl this week (2026-08-29, first re-crawl among the 5 since 08-17); the other four are unchanged. No Request Indexing action taken — all 5 already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (11th re-run, `scripts/_gsc-ctr-check-2026-08-31.mjs`, RECENT window now Aug 4–31): still unreadable at the query level for all 6 target queries — zero clicks in every window, every run, unchanged (11 straight checks) — but this was the most eventful week yet at the **page** level: `vrindavan/may` reversed last week's flattening with its largest single-week position gain yet (20.5 → **11.0**, impressions 12→15); `yercaud/may` is now on a genuine **3-week worsening trend** (position 11.3→16.4→**29.3** across 08-17/08-24/08-31, impressions 77→69→41) and its 4-week click streak broke (0 clicks this week); `darjeeling/june` moved for the first time in weeks (position 11.0 flat → **36.0**, on just 2 impressions — logged as a new data point, not a trend); `pondicherry/may` went fully quiet after 4 static weeks (1 impr/pos 47.0 → 0 impressions); `chakrata/may` logged a **4th straight week at 0 impressions**. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM) **for the 11th consecutive time** — the consolidation question itself is 11-for-11 unchanged and already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons; the one nuance is that the CTR/position-tracking half just produced its first real multi-week signal (yercaud's decline), worth a manual glance whenever GSC is next opened regardless of this task's fate. Left the task enabled since this run's brief didn't authorize disabling automations — founder decision needed to actually turn it off (see `gsc-audits/scheduled-2026-08-31-canonical-consolidation.md`).
