...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-08-17 (9th consecutive week of the disable recommendation, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) still show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). `yercaud/may` got a fresh Googlebot re-crawl this week (2026-08-14) — the first fresh crawl among the 5 since vrindavan's on 08-01, returning to the "usually one URL per week" pattern after last week's rare zero-crawl gap; the other 4 `lastCrawlTime`s are unchanged. No Request Indexing action taken — all 5 already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (9th re-run, `scripts/_gsc-ctr-check-2026-08-17.mjs`, RECENT window now Jul 21–Aug 17): still unreadable for 5 of 6 target queries (hard-seasonal "X in may" searches, 0% pre-window CTR baseline, re-test May 2027); zero clicks on all 6 named queries in every window, every run, unchanged (9 straight checks). Two threads continue: `vrindavan/may` page position improved a **third consecutive week** — 30.8 (08-03) → 24.3 (08-10) → **20.6 (08-17)** — on declining impressions (33→30→17, "may" season fading further), still deliberately not called "fixed" given the rolling 28-day window and the 08-03 run's experience correcting an over-confident "stabilized" call; `yercaud/may` logged a **third consecutive page-level click** (77 impr, 1 clk, 1.30% CTR) — still can't attribute it to the named query itself (0 clicks there, unchanged), and the 7-day-shifting 28-day windows overlap enough that this could be the same underlying click persisting rather than 3 distinct events. `pondicherry/may` is now static for a **third straight week**: identically 1 impression / 0 clicks / position 47.0 in the 08-03, 08-10, and 08-17 windows. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM) **for the 9th consecutive time**: the 2026-06-22 through 08-10 runs already reached the same operational conclusion (no action needed) — this week's movements (vrindavan's 3rd improving week, yercaud's 3rd click, pondicherry's 3rd static week) don't change that. Ongoing indexing health is already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons. Left the task enabled since this run's brief didn't authorize disabling automations — founder decision needed to actually turn it off (see `gsc-audits/scheduled-2026-08-17-canonical-consolidation.md`).
