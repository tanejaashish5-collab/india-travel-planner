...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-07-06 (5th consecutive weekly check, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). No Request Indexing action taken — the URLs are already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (3rd re-run, `scripts/_gsc-ctr-check-2026-07-06.mjs`) remains unreadable for 5 of 6 target queries: they're hard-seasonal "X in may" searches with pre-window CTR already at 0%, so there's no baseline to show lift against (re-test May 2027). The 6th, `darjeeling june weather`, is now resolved: the exact query string still shows 0 impressions in a fully-closed June, but its page (`/en/destination/darjeeling/june`) is healthy — 157 impressions and position improved 11.5→9.6 — traffic is just arriving via other query phrasings, not a ranking regression. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM): the 2026-06-22 and 2026-06-29 runs already reached this same conclusion and nothing has changed across 3 consecutive checks; ongoing indexing health is already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons.
