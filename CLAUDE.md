...
- **Any scheduled/headless audit session MUST `git commit` the audit files it writes — and if the web build must pick them up, push (empty commit if vercel-ignore skips it) — before ending.** `audit-snapshots.json` is built by the apps/web prebuild from COMMITTED audit files only; uncommitted audit runs silently freeze the GSC indexing snapshot. Bit us identically twice (2026-06-11, 2026-07-13); promoted to rule by dream proposal 2026-07-13.
...
- ✅ **GSC URL Inspection for top 5 non-prefixed URLs — RESOLVED, re-verified 2026-07-20 (7th consecutive weekly check, stable since 2026-05-18).** All 5 (kumbhalgarh/vrindavan/yercaud/chakrata/pondicherry × may) still show Google's selected canonical correctly pointing at `/en/` via `node scripts/gsc-inspect-sweep.mjs --url <url>` (no dashboard, no Chrome). New this run: `lastCrawlTime` for all 5 is byte-identical to 07-13 — Google isn't even re-crawling these anymore, the strongest possible "fully settled" signal. No Request Indexing action taken — the URLs are already consolidated, so submitting one would be a no-op that just spends GSC's daily quota. Apr-27-snippet-rewrite CTR check (5th re-run, `scripts/_gsc-ctr-check-2026-07-20.mjs`) is unchanged: still unreadable for 5 of 6 target queries (hard-seasonal "X in may" searches, 0% pre-window CTR baseline, re-test May 2027); `darjeeling june weather` remains resolved and stable — page position keeps improving (11.5→9.6→8.7→8.3), zero clicks across all 6 queries in every window, pre- and post-deploy alike. One page-level swing worth a glance next run: `vrindavan/may` aggregate position moved to 28.6 (from 15.6) on 26 impressions — read as low-volume noise since the actual target query has had 0 impressions since April, not a ranking regression. **Recommend disabling the `gsc-canonical-consolidation` scheduled task** (Mondays 9:05 PM) **for the 5th consecutive time**: the 2026-06-22, 06-29, 07-06, and 07-13 runs already reached this identical conclusion — 5 straight checks with zero new information. Ongoing indexing health is already covered by the weekly `gsc-inspect-sweep --patch` + `canary-probe` crons. Left the task enabled since this run's brief didn't authorize disabling automations — founder decision needed to actually turn it off (see `gsc-audits/scheduled-2026-07-20-canonical-consolidation.md`).
