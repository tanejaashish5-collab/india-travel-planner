#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-08-03 run.
Carried forward unchanged in methodology from qa/findings/2026-07-27/probe.py
(same soft-404 URL set, same core-route list, same R2 hero-image URL pattern,
same SEO check regexes) with date/seed bumped.
"""
...
def stage_section_f():
    def post(url, body, headers=None):
        try:
            ...
            return {"status": r.status_code, "bytes": len(r.content), "snippet": r.text[:400], "headers": dict(r.headers)}
        except Exception as e:
    ...
    tests.append(("chat_valid_1", "POST", lambda: post(f"{BASE}/api/chat", json.dumps({"question": "Best time to visit Leh?"}))))
    tests.append(("chat_valid_2_hello", "POST", lambda: post(f"{BASE}/api/chat", json.dumps({"question": "hello"}))))
    tests.append(("chat_valid_3_complex", "POST", lambda: post(f"{BASE}/api/chat", json.dumps({"question": "Compare Manali vs Shimla for a family trip, pros and cons"}))))
