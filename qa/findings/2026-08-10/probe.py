#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-08-10 run.
Carried forward from qa/findings/2026-08-03/probe.py (same soft-404 URL set,
same core-route baseline, same R2 hero-image URL pattern, same SEO regexes)
with two structural changes reflecting the 2026-08-04 metered-AI removal
(commits 444b37f9 / 6143c6ea):
  - Section F: /api/chat + /api/gap-year/* + /api/embed replaced with
    404-confirmation probes (routes were deleted, not fixed); added an
    itinerary_valid probe since /api/itinerary is now the only AI-adjacent
    surface left and had never been end-to-end tested post-refactor.
  - Section I: /en/ask dropped from the core-route tally (page deleted by
    design) — its removal is verified explicitly in Section F instead.
"""
...
def stage_section_f():
    def post(url, body, headers=None):
        try:
            ...
            return {"status": r.status_code, "bytes": len(r.content), "snippet": r.text[:500], "headers": dict(r.headers)}
        except Exception as e:
    ...
    tests = []
    # --- AI-surface-removal confirmation (2026-08-04 refactor: 444b37f9 / 6143c6ea) ---
    # These routes were DELETED, not fixed. Expect 404, not 500/200.
    ...
    # --- itinerary (kept, now deterministic-only per route.ts:403-410) ---
    ...
    tests.append(("itinerary_valid", "POST", lambda: post(f"{BASE}/api/itinerary", json.dumps({"month": 12, "days": 5, "travelerType": "family", "budget": "mid-range"}))))
    ...
    # Validate itinerary_valid response shape (deterministic-fallback correctness check)
    iv = next(r for r in rows if r["name"] == "itinerary_valid")
    try:
        body = json.loads(iv.get("snippet", "") if len(iv.get("snippet", "")) < 500 else "{}")
    except Exception:
        body = {}
    validation = {"status": iv.get("status")}
...
def stage_section_i():
    # /en/ask dropped 2026-08-10: page deleted 2026-08-04 (444b37f9), removal
    # verified explicitly in Section F (removed_ask_page). Keeping it here would
    # mislabel an intentional removal as a core-route failure.
    routes = [
