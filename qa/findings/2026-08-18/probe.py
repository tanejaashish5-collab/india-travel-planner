#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-08-15 run.
Carried forward from qa/findings/2026-08-10/probe.py, incorporating the
2026-08-13 run's methodology upgrades (not previously captured as a script
since 08-13 had no artifact folder):
  - Section A: added /en/family/not-a-real-state-999 bad-state probe.
  - Section C: added H1-language + translated-section-heading checks to
    precisely scope NEW-2026-07-09-001 (cinematic vs non-cinematic split).
  - Section F: itinerary_valid expanded to 3 varied probes (incl. explicit
    destinationIds) to re-verify the quality-filter behaviour, not just 200.
  - Section I: widened core-route list (membership, saved, family/kerala,
    state/kerala, vs/manali-vs-shimla, with-kids/manali, 3 weekend-from-city
    routes, hi/quiz) per 08-13's expansion.
  - Section family fix regression check: uses the CORRECT /en/with-kids/<slug>
    href pattern (08-13 self-caught a methodology bug using /en/destination/).
"""
...
def stage_family_fix_check():
    # 08-13's self-caught methodology bug: family pages link to /en/with-kids/<slug>,
    # NOT /en/destination/<slug>. Re-verify with the CORRECT pattern this time.
    rows = []
...
if __name__ == "__main__":
    ...
    print("=== L (http fallback, real script run separately) ==="); stage_section_l()
