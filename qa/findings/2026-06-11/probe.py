#!/usr/bin/env python3
"""NakshIQ daily QA probe — 2026-06-11. Sections A,B,C,D,E,F,G,H,I,J.
Sequential honest timing via curl. Evidence CSVs written alongside.
Usage: python3 probe.py <section>   (section in a,b,c,d,e,f,g,h,i,j,sample)
"""
...
UA = "NakshIQ-DailyQA/2026-06-11 (curl probe; Cowork scheduled task)"
BODYF = "/tmp/_body_20260611"

def curl(url, method="GET", data=None, headers=None, max_time=60):
    """Return (http_code, ttfb_ms, total_ms, size, body)."""
    cmd = ["curl", "-sS", "-o", BODYF, "-w",
...
def get_sample():
    ...
    slugs = sorted(slugs)
    random.seed("nakshiq-2026-06-11")
...
def sec_b():
    ...
    w("section-B-availability.csv", rows, ["slug", "status", "ttfb_ms", "total_ms", "bytes"])
...
def sec_c():
    ...
    w("section-C-hindi.csv", rows, ["slug", "status", "html_lang", "title_devanagari", "title", "ttfb_ms"])
...
def sos_blob(body):
    ...
    # unescape the RSC string escaping: \" -> " and \\ -> \  (order matters)
...
def phone_ok(v):
    ...
    digits = re.sub(r"\D", "", v)
    return len(digits) >= 2                                # dialable-ish (108, 112, full numbers, multi-number text)
...
def sec_e():
    ...
    for s in sample:
        ...
        total_fields += n_struct; malformed += len(bad)
        if nodigit: textonly.append((s, nodigit))
        has112 = "112" in body
        rows.append([s, n_struct, ";".join(sorted(found.keys())), len(bad), ";".join(bad),
                     ";".join(nodigit), len(helper_phones), has112])
        print(s, f"structured={n_struct} malformed={len(bad)} textonly={nodigit} helper_phones={len(helper_phones)}")
        for f, v in sorted(found.items()):
            print(f"   {f} = {v[:70]}")
    w("section-E-sos.csv", rows, ["slug", "structured_fields", "field_names", "malformed_count",
                                   "malformed_fields", "text_no_digit_fields", "local_helper_phones", "universal_112_present"])
    print(f"TOTAL structured fields: {total_fields}, malformed: {malformed}, text-only: {textonly}")
    print(f"DESTS WITH NO emergencySos BLOB: {no_blob if no_blob else 'none'}")

# ---------- F: API smoke ----------
def sec_f():
    open("/tmp/_oversized_20260611.json", "w").write("x" * 200000)
    probes = [
        ...
        ("POST /api/chat oversized-200KB", f"{BASE}/api/chat", "POST", "@/tmp/_oversized_20260611.json", ["Content-Type: application/json"]),
    ...
    w("section-F-api.csv", rows, ["probe", "status", "bytes", "body_first150"])
...
def sec_g():
    ...
    if ver:
        class V: pass
        v = V(); v.group = lambda _=0, g=ver.group(1): g; ver = v
    else:
        all_v = re.findall(r"nakshiq-v(\d+)", body)
        if all_v:
            class V2: pass
            v = V2(); top = "nakshiq-v" + str(max(int(x) for x in all_v)); v.group = lambda _=0, g=top: g; ver = v
    md5 = hashlib.md5(body.encode()).hexdigest()
    rows.append(["/sw.js", code, size, ver.group(0) if ver else "?", md5])
    print("/sw.js", code, size, ver.group(0) if ver else "?", "md5", md5)
...
def sec_i():
    ...
    med = int(statistics.median(ttfbs)); p90 = sorted(ttfbs)[int(len(ttfbs)*0.9)-1]
    print(f"TTFB min={min(ttfbs)} median={med} p90={p90} max={max(ttfbs)}")
    w("section-I-core-routes.csv", rows, ["route", "final_status", "ttfb_ms", "total_ms", "bytes", "num_redirects", "redirected_to"])
...
if __name__ == "__main__":
    ...
    {"sample": get_sample, "a": sec_a, "b": sec_b, "c": sec_c, "d": sec_d,
     "e": sec_e, "f": sec_f, "g": sec_g, "h": sec_h, "i": sec_i, "j": sec_j}[sec]()
