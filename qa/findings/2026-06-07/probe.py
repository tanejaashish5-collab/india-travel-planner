#!/usr/bin/env python3
"""NakshIQ daily QA probe — 2026-06-07. Sections A,B,C,D,E,F,G,H,I,J.
...
def get_sample():
    ...
    slugs = sorted(slugs)
    random.seed("nakshiq-2026-06-07")
...
# ---------- F: API smoke ----------
def sec_f():
    open("/tmp/_oversized.json", "w").write("x" * 200000)
    probes = [
        ...
        ("POST /api/chat oversized-200KB", f"{BASE}/api/chat", "POST", "@/tmp/_oversized.json", ["Content-Type: application/json"]),
...
def sec_g():
    ...
    ver = re.search(r"nakshiq-v\d+", body)
