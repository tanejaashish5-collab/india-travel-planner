#!/usr/bin/env python3
"""NakshIQ daily QA probe — 2026-07-12. Sections A,B,C,D,E,F,G,H,I,J.
...
Adapted from qa/findings/2026-07-11/probe.py — same methodology, new date/seed.
"""
...
UA = "NakshIQ-DailyQA/2026-07-12 (curl probe; Cowork scheduled task)"
BODYF = "/tmp/_body_20260712"
...
def get_sample():
    ...
    slugs = sorted(slugs)
    random.seed("nakshiq-2026-07-12")
...
def sec_f(part=0, _unused=None):
    open("/tmp/_oversized_20260712.json", "w").write("x" * 200000)
    p1 = [
        ...
        ("POST /api/chat oversized-200KB", f"{BASE}/api/chat", "POST", "@/tmp/_oversized_20260712.json", ["Content-Type: application/json"]),
...
# ---------- F2: deploy-freshness (data-dpl-id) ----------
def sec_f_deploy():
    code, _, _, size, body, _, _ = curl(f"{BASE}/api/health")
    m = re.search(r'data-dpl-id="([^"]+)"', body)
    dpl = m.group(1) if m else None
    print(f"/api/health -> {code} ({size}b) data-dpl-id={dpl}")
    w("section-F2-deploy.csv", [["/api/health", code, size, dpl or ""]], ["url", "status", "bytes", "data_dpl_id"])
...
if __name__ == "__main__":
    ...
    fn = {"sample": get_sample, "a": sec_a, "b": sec_b, "c": sec_c, "d": sec_d,
          "d2": sec_d2, "e": sec_e, "f": sec_f, "fdeploy": sec_f_deploy, "g": sec_g, "h": sec_h,
