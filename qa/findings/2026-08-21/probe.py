#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-08-21 run.
Carried forward verbatim in methodology from 2026-08-20/probe.py (the most
recent COMPLETE prior run). Only the SEED/date changed.
"""
    ...
    for r in rows:
        ...
        for k, v in h.items():
            if "dpl" in str(v):
                ...
                dpl_source = "header"
    if dpl is None:
        # FIX (2026-08-21, per 08-20 honest_caveats housekeeping note): data-dpl-id lives in the
        # response BODY of error/404 pages, not header values. Header-only grep printed a false
        # "None" every prior run despite the id being trivially readable in the same response body.
        for r in rows:
