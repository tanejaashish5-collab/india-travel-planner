#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-08-27 run.
Carried forward verbatim in methodology from 2026-08-26/probe.py (the most
recent COMPLETE prior run). SEED/date changed per the standing copy-forward
convention, plus one addition: a GET smoke probe for the new
/api/webhooks/resend route (shipped 5e10e8cc, 2026-08-26 evening) since a
real deploy landed since yesterday's QA run.
"""
...
def stage_section_f():
    ...
    tests.append(("removed_export_trip_get", "GET", lambda: get_(f"{BASE}/api/export-trip")))  # added 2026-08-24: route removed bdb9f02a, verify stays 404
    ...
    tests.append(("itinerary_unresolvable_destination_ids", "POST", lambda: post(f"{BASE}/api/itinerary", json.dumps({"month": 6, "days": 3, "travelerType": "solo", "budget": "budget", "destinationIds": ["totally-fake-slug-xyz-123", "another-fake-999"]}))))  # added 2026-08-24: expect 404 per fix 354eed17
    ...
    # NEW 2026-08-27: /api/webhooks/resend shipped 5e10e8cc (2026-08-26 evening) + hardened 5360533e.
    # GET has no handler -> expect 405. POST with no/garbage svix headers -> expect 401 "Invalid signature" (fail-closed), never 5xx.
