#!/usr/bin/env python3
"""NakshIQ daily QA probe script — 2026-09-07 run.
Carried forward verbatim in methodology from 2026-09-04/probe.py (the most
recent prior run). SEED/date changed only.
"""
...
def stage_section_f():
    ...
    tests.append(("removed_export_trip_get", "GET", lambda: get_(f"{BASE}/api/export-trip")))
    ...
    tests.append(("itinerary_unresolvable_destination_ids", "POST", lambda: post(f"{BASE}/api/itinerary", json.dumps({"month": 6, "days": 3, "travelerType": "solo", "budget": "budget", "destinationIds": ["totally-fake-slug-xyz-123", "another-fake-999"]}))))
    ...
    tests.append(("webhook_resend_get", "GET", lambda: get_(f"{BASE}/api/webhooks/resend")))
    tests.append(("webhook_resend_post_no_sig", "POST", lambda: post(f"{BASE}/api/webhooks/resend", json.dumps({"type": "email.opened", "data": {"to": "qa-probe@example.com"}}))))
    tests.append(("webhook_resend_post_garbage_sig", "POST", lambda: post(f"{BASE}/api/webhooks/resend", json.dumps({"type": "email.opened"}), headers={"Content-Type": "application/json", "svix-id": "qa-probe", "svix-timestamp": str(int(time.time())), "svix-signature": "v1,not-a-real-signature"})))
...
def stage_regression_state_slug_collision():
    # Targeted re-check of NEW-2026-09-05-001: destination slugs colliding with
    # an Indian state/UT name bypassing soft-404 on garbage month sub-routes.
    colliding = ["puducherry", "goa", "sikkim", "delhi", "chandigarh"]
    noncolliding = ["anamalai", "amritsar"]
    urls = []
    for s in colliding + noncolliding:
        urls.append(f"{BASE}/en/destination/{s}/99")
        urls.append(f"{BASE}/hi/destination/{s}/99")
    rows = []
    with ThreadPoolExecutor(max_workers=8) as ex:
        futs = {ex.submit(probe_one, u): u for u in urls}
        for fut in as_completed(futs):
            ...
            rows.append({"url": u, "status": res.get("status"), "bytes": res.get("bytes"), "noindex_meta": noindex})
    rows.sort(key=lambda r: urls.index(r["url"]))
    write_csv("section-A2-state-slug-collision-regression.csv", rows, ["url", "status", "bytes", "noindex_meta"])
    print(rows)
...
if __name__ == "__main__":
    ...
    print("=== regression state-slug-collision ==="); stage_regression_state_slug_collision()
