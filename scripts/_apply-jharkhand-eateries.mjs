...
const files = readdirSync(dir).filter(
  (f) => f.startsWith("jharkhand-") && f.endsWith(".json")
);
console.log(`Loading ${files.length} jharkhand-*.json files`);

const targetDests = new Set(["deoghar", "netarhat", "ranchi", "shikharji"]);
...
if (dropped.length)
  for (const d of dropped) console.log(`  DROP: [${d.file}] ${d.name} — ${d.reason}`);
...
const { data, error } = await s
  .from("local_eateries")
  .upsert(rows, { onConflict: "destination_id,name,area" })
  .select("id");
if (error) {
