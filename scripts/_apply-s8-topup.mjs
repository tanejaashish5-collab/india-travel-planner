#!/usr/bin/env node
// S8 applier — same shape as S7. Reuses gem dual-shape, eat int4range, stay transforms.
...
const gemFiles = ["data/research/s8-gems-2026-05-09.json"];
const eatFiles = ["data/research/eateries/s8-2026-05-09.json"];
const stayFiles = ["data/research/s8-stays-2026-05-09.json"];
...
if (gems.length) {
  ...
  if (error) { console.error("gems:", error); process.exit(1); }
...
if (eats.length) {
  ...
  const newEats = eats
    ...
    .map(cleanEat);
  console.log(`eats new: ${newEats.length}`);
  if (newEats.length) {
    const { error } = await supabase.from("local_eateries").insert(newEats);
    if (error) { console.error("eats:", error.message); process.exit(1); }
...
if (stays.length) {
  ...
  if (error) { console.error("stays:", error); process.exit(1); }
