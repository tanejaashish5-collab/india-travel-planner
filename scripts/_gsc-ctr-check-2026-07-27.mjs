...
// One-off (gen 2026-07-27 by scheduled gsc-canonical-consolidation task):
// 6th re-run of the Apr 27 snippet-rewrite CTR check. Prior runs
// (2026-06-22, 2026-06-29, 2026-07-06, 2026-07-13, 2026-07-20) all concluded
// the same thing: 5 of 6 target queries are hard-seasonal "X in may" searches
// with 0% pre-window CTR, so there's no baseline to show lift against
// (re-test May 2027); the 6th ("darjeeling june weather") was resolved
// 2026-07-06 — 0 impressions on the exact query string but the page itself
// is healthy and its position keeps improving each check.
// This run only refreshes the RECENT window (Jun 30 - Jul 27, 28d) to check
// for drift since 07-20; pre/june-full windows kept identical for
...
const WINDOWS = [
  ...
  { key: "recent", label: "RECENT28d (Jun 30 - Jul 27)", startDate: "2026-06-30", endDate: "2026-07-27" },
];
...
console.log(`GSC CTR check — Apr 27 snippet rewrite (6th re-run)\n`);
console.log(`Property: ${SITE_URL}`);
console.log(`Windows: PRE (Mar 30-Apr 26) vs JUNE-FULL (Jun 1-30) vs RECENT28d (Jun 30-Jul 27)\n`);
