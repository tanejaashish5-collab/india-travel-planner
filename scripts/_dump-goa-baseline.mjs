...
const { data: dests } = await s.from("destinations").select("id, name, tagline, region").eq("state_id", "goa").order("id");
...
writeFileSync("/Users/ashishtaneja/Desktop/India Travel Planner/data/research/_goa-baseline.json", JSON.stringify(byDest, null, 2));
console.log(`Wrote baseline for ${dests.length} Goa dests. Totals: ${e?.length ?? 0} eateries · ${ls?.length ?? 0} stays · ${p?.length ?? 0} picks`);
