...
// pick first row to discover column names
console.log("pick columns:", p && p[0] ? Object.keys(p[0]) : "no rows");
...
writeFileSync(
  "/Users/ashishtaneja/Desktop/India Travel Planner/data/research/eateries/_gujarat-baseline.json",
  JSON.stringify(byDest, null, 2),
);

console.log(`Wrote baseline for ${dests.length} Gujarat dests.`);
console.log(`Totals: ${e?.length ?? 0} eateries · ${ls?.length ?? 0} stays · ${p?.length ?? 0} picks`);
