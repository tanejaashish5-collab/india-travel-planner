...
const { data: dests } = await s.from("destinations").select("id, name, tagline, region").eq("state_id", "odisha").order("id");
...
writeFileSync("/Users/ashishtaneja/Desktop/India Travel Planner/data/research/_odisha-baseline.json", JSON.stringify(byDest, null, 2));
console.log(`Wrote baseline for ${dests.length} Odisha dests. Totals: ${e?.length ?? 0} eateries · ${ls?.length ?? 0} stays · ${p?.length ?? 0} picks`);
