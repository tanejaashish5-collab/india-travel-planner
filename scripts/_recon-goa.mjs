...
const { data: dests } = await s.from("destinations").select("id, name, region, tagline").eq("state_id", "goa").order("id");
console.log(`Goa destinations: ${dests.length}\n`);
...
console.log("dest".padEnd(22) + "eat  stay  picks  picks-slots".padEnd(45) + " 1st eatery quality signal");
...
for (const d of dests) {
  ...
  console.log(d.id.padEnd(22) + String(e.length).padStart(3) + String(st.length).padStart(6) + String(p.length).padStart(7) + "  " + slots.padEnd(35) + " " + sig.slice(0, 70));
}
...
const dateBuckets = {};
for (const r of eCounts ?? []) {
  const key = r.last_verified ?? "NULL";
  dateBuckets[key] = (dateBuckets[key] ?? 0) + 1;
}
console.log("\nlast_verified buckets:");
for (const d of Object.keys(dateBuckets).sort()) console.log(`  ${d}: ${dateBuckets[d]}`);
