...
// One-off (gen 2026-09-14 by scheduled gsc-canonical-consolidation task):
// 13th re-run of the Apr 27 snippet-rewrite CTR check. Prior runs
...
// all concluded the same thing: 5 of 6 target queries are hard-seasonal
// "X in may" searches with 0% pre-window CTR, so there's no baseline to show
// lift against (re-test May 2027); the 6th ("darjeeling june weather") was
// resolved 2026-07-06 — 0 impressions on the exact query string but the page
// itself is healthy.
// This run only refreshes the RECENT window (Aug 18 - Sep 14, 28d) to check
// for drift since 09-07; pre/june-full windows kept identical to all prior
...
const WINDOWS = [
  ...
  { key: "recent", label: "RECENT28d (Aug 18 - Sep 14)", startDate: "2026-08-18", endDate: "2026-09-14" },
];
...
console.log(`GSC CTR check — Apr 27 snippet rewrite (13th re-run)\n`);
console.log(`Property: ${SITE_URL}`);
console.log(`Windows: PRE (Mar 30-Apr 26) vs JUNE-FULL (Jun 1-30) vs RECENT28d (Aug 18-Sep 14)\n`);
