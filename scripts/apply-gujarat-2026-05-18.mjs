...
// Apply Gujarat eateries + stays + picks (DELETE placeholders → INSERT verified).
// Usage:
//   node scripts/apply-gujarat-2026-05-18.mjs --dry-run
//   node scripts/apply-gujarat-2026-05-18.mjs --commit
...
// ---- Load Gujarat destination IDs ----
const { data: dests } = await s.from("destinations").select("id, name").eq("state_id", STATE).order("id");
const destIds = dests.map((d) => d.id);
console.log(`Gujarat: ${destIds.length} destinations`);

// ---- Load eateries JSONs (gujarat-*.json) ----
const EAT_DIR = path.join(ROOT, "data", "research", "eateries");
const eatFiles = readdirSync(EAT_DIR).filter((f) => f.startsWith("gujarat-") && f.endsWith(".json"));
const eateries = [];
for (const f of eatFiles) {
  const rows = JSON.parse(readFileSync(path.join(EAT_DIR, f), "utf-8"));
  if (!Array.isArray(rows)) throw new Error(`${f} not an array`);
  for (const r of rows) {
    if (!destIds.includes(r.destination_id)) {
      console.warn(`  ⚠ ${f}: skipping ${r.name} — dest "${r.destination_id}" not in Gujarat DB`);
      continue;
    }
    // Sanitize maps URL — replace shortlinks with name-search URLs
...
// ---- Load stays JSON ----
const STAYS_FILE = path.join(ROOT, "data", "research", "stays", "gujarat-new-stays-2026-05-18.json");
const stays = JSON.parse(readFileSync(STAYS_FILE, "utf-8"));
console.log(`Stays to insert: ${stays.length}`);

// ---- Load picks JSON + transform task_a_results → picks rows ----
const PICKS_FILE = path.join(ROOT, "data", "research", "stays-audit", "gujarat-replacements-2026-05-18.json");
...
for (const t of picksAudit.task_a_results ?? []) {
  ...
  if (!destIds.includes(t.destination_id)) {
    console.warn(`  ⚠ pick: skipping ${t.destination_id}/${t.slot} — not in Gujarat DB`);
    continue;
  }
