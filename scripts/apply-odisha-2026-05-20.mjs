...
// Apply Odisha eateries + stays + picks. DELETE existing placeholders → INSERT verified.
// Usage:
//   node scripts/apply-odisha-2026-05-20.mjs --dry-run
//   node scripts/apply-odisha-2026-05-20.mjs --commit
...
console.log(`Odisha: ${destIds.length} destinations`);

const EAT_DIR = path.join(ROOT, "data", "research", "eateries");
const eatFiles = readdirSync(EAT_DIR).filter((f) => f.startsWith("odisha-") && f.endsWith(".json"));
const eateries = [];
for (const f of eatFiles) {
  ...
  for (const r of rows) {
    if (!destIds.includes(r.destination_id)) { console.warn(`  ⚠ ${f}: dest "${r.destination_id}" not in Odisha DB`); continue; }
...
const STAYS_FILE = path.join(ROOT, "data", "research", "stays", "odisha-new-stays-2026-05-20.json");
const stays = JSON.parse(readFileSync(STAYS_FILE, "utf-8"));
console.log(`Stays to insert: ${stays.length}`);

const PICKS_FILE = path.join(ROOT, "data", "research", "stays-audit", "odisha-replacements-2026-05-20.json");
...
for (const t of picksAudit.task_a_results ?? []) {
  ...
  if (!destIds.includes(t.destination_id)) { console.warn(`  ⚠ pick ${t.destination_id}/${t.slot} dest not in Odisha DB`); continue; }
