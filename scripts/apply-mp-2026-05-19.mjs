...
// Apply Madhya Pradesh eateries + stays + picks. DELETE existing → INSERT verified.
// Usage:
//   node scripts/apply-mp-2026-05-19.mjs --dry-run
//   node scripts/apply-mp-2026-05-19.mjs --commit
...
console.log(`Madhya Pradesh: ${destIds.length} destinations`);

const EAT_DIR = path.join(ROOT, "data", "research", "eateries");
const eatFiles = readdirSync(EAT_DIR).filter((f) => f.startsWith("madhya-pradesh-") && f.endsWith(".json"));
const eateries = [];
for (const f of eatFiles) {
  ...
  for (const r of rows) {
    if (!destIds.includes(r.destination_id)) { console.warn(`  ⚠ ${f}: dest "${r.destination_id}" not in MP DB`); continue; }
...
const STAYS_FILE = path.join(ROOT, "data", "research", "stays", "madhya-pradesh-new-stays-2026-05-19.json");
const stays = JSON.parse(readFileSync(STAYS_FILE, "utf-8"));
console.log(`Stays to insert: ${stays.length}`);

const PICKS_FILE = path.join(ROOT, "data", "research", "stays-audit", "madhya-pradesh-replacements-2026-05-19.json");
...
for (const t of picksAudit.task_a_results ?? []) {
  ...
  if (!destIds.includes(t.destination_id)) { console.warn(`  ⚠ pick ${t.destination_id}/${t.slot}`); continue; }
