...
// Apply Telangana eateries + stays + picks. DELETE existing placeholders → INSERT verified.
// Usage:
//   node scripts/apply-telangana-2026-05-20.mjs --dry-run
//   node scripts/apply-telangana-2026-05-20.mjs --commit
...
console.log(`Telangana: ${destIds.length} destinations`);

const EAT_DIR = path.join(ROOT, "data", "research", "eateries");
const eatFiles = readdirSync(EAT_DIR).filter((f) => f.startsWith("telangana-") && f.endsWith(".json"));
const eateries = [];
for (const f of eatFiles) {
  ...
  for (const r of rows) {
    if (!destIds.includes(r.destination_id)) { console.warn(`  ⚠ ${f}: dest "${r.destination_id}" not in Telangana DB`); continue; }
...
const STAYS_FILE = path.join(ROOT, "data", "research", "stays", "telangana-new-stays-2026-05-20.json");
const stays = JSON.parse(readFileSync(STAYS_FILE, "utf-8"));
console.log(`Stays to insert: ${stays.length}`);

const PICKS_FILE = path.join(ROOT, "data", "research", "stays-audit", "telangana-replacements-2026-05-20.json");
...
for (const t of picksAudit.task_a_results ?? []) {
  ...
  if (!destIds.includes(t.destination_id)) { console.warn(`  ⚠ pick ${t.destination_id}/${t.slot} dest not in Telangana DB`); continue; }
