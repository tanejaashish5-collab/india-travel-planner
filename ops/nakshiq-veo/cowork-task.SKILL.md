...
6. SAME PEOPLE WITHIN A REEL. Every clip has a `character` string, identical across all beats of one storyboard. Put that exact text in Flow's character field for each of that storyboard's beats. A character set in one project does NOT carry to another, so when a storyboard's beats straddle two accounts, re-enter the same character text in the new account's project. Different storyboards get different people; that is intended.
