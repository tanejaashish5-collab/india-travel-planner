...
- **Business DNA editor mechanics (learned scripting the Sindoor DNA edit on 2026-09-02, all verified live):** edit icons are Material ligature leaves inside Angular `app-*-card` components with NO `<button>` ancestor — mouse-click the leaf's bounding-rect centre after `scrollIntoView({block:'center'})` (the Colors card hides under the sticky Reset/Brand Book bar). Location/Phone/Keywords/Social/CTA/Testimonials live under the **Business Details** tab. Editors are CDK overlays (`.cdk-overlay-container`) with an `Apply` button that is DISABLED until something changes. **Apply only updates local state; Pomelli autosaves a few seconds later — navigate within ~10 s and the change is silently lost.** Font picker: clicking a result REPLACES the existing chip; click **Add font** first to get the second slot; the last remaining chip has no remove button; result rows only match via a shadow-piercing text locator after the list stops showing "Loading…"; the searchable list lacked Outfit/Poppins/Jost/DM Sans/Manrope. Colour swatch click opens a nested "Pick a color" pane with `input[aria-label="HEX color"]` (type hex + Enter); that pane's close is the LAST `close` button in the overlay and must be closed before Apply. Sindoor DNA now: colours #efe7d9 #090504 #e0ae57 #bb061e #ffffff, fonts Cormorant Garamond + Inter, location/phone/email/CTA/keywords filled.
