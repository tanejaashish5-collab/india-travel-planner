...
6. **The Ratan Tata Final Gate (ALWAYS LAST — it overrides everything).** The strategist checklist judges whether an idea will *win*; the Ratan Tata gate judges whether it *should exist*. It is the final word: a `fail` BLOCKS the idea regardless of conviction; a `conditional` does NOT demote — the gate's fix is carried as a mandatory launch precondition (founder 2026-06-11: the gate is an intelligent ethics layer that FINDS the ethical path, never breaks a profitable idea for no reason). If `data/research/ratan-tata-gate.json` exists, run every idea through it:
